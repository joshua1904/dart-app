...
class MultiplayerGameTests(TestCase):
    ...
    def test_get_game_context(self):
        ...
        self.assertEqual(context["turn"].rank, 1)
        self.assertEqual(
            context["left_score"], 50
        )  # Player 1 has 50 left from previous round
    ...
    def test_get_wins_authenticated_user_with_wins(self):
        ...
        player1_game1 = MultiplayerPlayer.objects.create(
            game=game1, rank=1, player=self.user1
        )
        player2_game1 = MultiplayerPlayer.objects.create(
            game=game1, rank=2, player=self.user2
        )

        player1_game2 = MultiplayerPlayer.objects.create(
            game=game2, rank=1, player=self.user1
        )
        player2_game2 = MultiplayerPlayer.objects.create(
            game=game2, rank=2, player=self.user2
        )

        player1_game3 = MultiplayerPlayer.objects.create(
            game=game3, rank=1, player=self.user1
        )
        player2_game3 = MultiplayerPlayer.objects.create(
            game=game3, rank=2, player=self.user2
        )
    ...
    def test_get_wins_guest_player_no_wins(self):
        ...
        # Create guest player
        guest_player = MultiplayerPlayer.objects.create(
            game=game, rank=1, guest_name="GuestPlayer"
        )
    ...
    def test_get_wins_guest_player_with_wins(self):
        ...
        guest1_game1 = MultiplayerPlayer.objects.create(
            game=game1, rank=1, guest_name="Guest1"
        )
        guest2_game1 = MultiplayerPlayer.objects.create(
            game=game1, rank=2, guest_name="Guest2"
        )

        guest1_game2 = MultiplayerPlayer.objects.create(
            game=game2, rank=1, guest_name="Guest1"
        )
        guest2_game2 = MultiplayerPlayer.objects.create(
            game=game2, rank=2, guest_name="Guest2"
        )

        guest1_game3 = MultiplayerPlayer.objects.create(
            game=game3, rank=1, guest_name="Guest1"
        )
        guest2_game3 = MultiplayerPlayer.objects.create(
            game=game3, rank=2, guest_name="Guest2"
        )
    ...
    def test_get_wins_mixed_players(self):
        ...
        auth_player_game1 = MultiplayerPlayer.objects.create(
            game=game1, rank=1, player=self.user1
        )
        guest_player_game1 = MultiplayerPlayer.objects.create(
            game=game1, rank=2, guest_name="TestGuest"
        )

        # Game 2: same players
        auth_player_game2 = MultiplayerPlayer.objects.create(
            game=game2, rank=1, player=self.user1
        )
        guest_player_game2 = MultiplayerPlayer.objects.create(
            game=game2, rank=2, guest_name="TestGuest"
        )
    ...
    def test_get_game_context_includes_wins(self):
        ...
        auth_player = MultiplayerPlayer.objects.create(
            game=game, rank=1, player=self.user1
        )
        guest_player = MultiplayerPlayer.objects.create(
            game=game, rank=2, guest_name="TestGuest"
        )
        auth_player2 = MultiplayerPlayer.objects.create(
            game=game, rank=3, player=self.user2
        )
        ...
        prev_auth_player1 = MultiplayerPlayer.objects.create(
            game=prev_game1, rank=1, player=self.user1
        )
        prev_guest_player1 = MultiplayerPlayer.objects.create(
            game=prev_game1, rank=2, guest_name="TestGuest"
        )
        prev_game1.winner = prev_auth_player1  # user1 wins
        prev_game1.save()

        prev_game2 = self.create_game(score=100, max_players=2, session=session)
        prev_auth_player2 = MultiplayerPlayer.objects.create(
            game=prev_game2, rank=1, player=self.user1
        )
        prev_guest_player2 = MultiplayerPlayer.objects.create(
            game=prev_game2, rank=2, guest_name="TestGuest"
        )
        prev_game2.winner = prev_guest_player2  # guest wins
        prev_game2.save()

        prev_game3 = self.create_game(score=100, max_players=2, session=session)
        prev_auth_player3 = MultiplayerPlayer.objects.create(
            game=prev_game3, rank=1, player=self.user2
        )
        prev_guest_player3 = MultiplayerPlayer.objects.create(
            game=prev_game3, rank=2, guest_name="TestGuest"
        )
        ...
        guest_in_queue = next(
            p for p in context["queue"] if p["player"].guest_name == "TestGuest"
        )
        auth2_in_queue = next(
            p for p in context["queue"] if p["player"].player == self.user2
        )
